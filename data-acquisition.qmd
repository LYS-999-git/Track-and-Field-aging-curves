...
Here is a script to get data from the World Athletics GraphQL API.
...
First, we scrape the results from the last 8 Olympic Games.

```{r}
library(rvest)

# first, I need to get the competition ID's for the last 8 Olympic Games, which I did manually
...
event_days <-   
 ...
 results <- 
   map(event_days, ~ scrape_results(competition_id, .x)) %>% 
   bind_rows() %>% 
   unnest_wider(data) %>% 
...
Now we scrape the career results for each athlete using the `olympics_data.csv` dataset
...
write_csv(all_results, "data.csv")
